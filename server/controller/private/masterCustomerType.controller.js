...
// GET
const loadMasterCustomerType = async (req, res) => {
  try {
    let sql = SelectAllStatement(
      Master.master_customer_type.tablename,
      Master.master_customer_type.selectColumns
    );
    ...
    Select(sql, (err, result) => {
      ...
      if (result != 0) {
        let data = DataModeling(result, Master.master_customer_type.prefix_);
...
// GET BY ID
const getMasterCustomerType = async (req, res) => {
  try {
    const { id } = req.params;
...
// POST
const addMasterCustomerType = async (req, res) => {
  try {
    ...
    const sql = InsertStatementTransCommit(
      Master.master_customer_type.tablename,
      Master.master_customer_type.prefix,
      Master.master_customer_type.insertColumns
    );
...
// PUT
const editMasterCustomerType = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    let sql = UpdateStatement(
      ...
      Master.master_customer_type.prefix,
      [
        Master.master_customer_type.updateOptionColumns.type_name,
        Master.master_customer_type.updateOptionColumns.description,
        Master.master_customer_type.updateOptionColumns.update_date,
        Master.master_customer_type.updateOptionColumns.create_by,
        Master.master_customer_type.updateOptionColumns.status,
      ],
      [Master.master_customer_type.updateOptionColumns.id]
    );
    ...
    let checkStatement = SelectStatement(
      "SELECT * FROM master_customer_type WHERE mct_id = ? AND mct_type_name = ? AND mct_description = ? AND mct_status = ?",
