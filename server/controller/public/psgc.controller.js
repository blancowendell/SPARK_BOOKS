...
const OUTPUT_DIR = path.join(__dirname, "../../database/seeders/data");
const CONCURRENCY_LIMIT = 6; // safe for PSGC Cloud
...
const writeJSON = (fileName, data) => {
  fs.writeFileSync(
    path.join(OUTPUT_DIR, fileName),
    JSON.stringify(data) // ⚡ faster, smaller
  );
};
...
const fetchAllPages = async (label, baseUrl, limit) => {
  ...
  console.log(`🏁 FINISHED ${label.toUpperCase()} | total ${results.length}\n`);
  return results;
};

/* ================= BARANGAYS (FAST PATH) ================= */

const fetchBarangaysByCity = async (cities) => {
  console.log(`\n▶️ START BARANGAYS (BY CITY)`);

  const barangays = [];
  const cityChunks = chunkArray(cities, CONCURRENCY_LIMIT);

  for (const chunk of cityChunks) {
    const requests = chunk.map(city =>
      axiosSafe
    ...
    console.log(
      `   ✅ Barangays fetched so far: ${barangays.length}`
    );
  }

  console.log(
    `🏁 FINISHED BARANGAYS | total ${barangays.length}\n`
  );
...
const generatePSGCJson = async (req, res) => {
  try {
    ...
    /* ========== BARANGAYS (OPTIMIZED) ========== */
    const barangays = await fetchBarangaysByCity(cities);
